  ...
  )
{
  ...
  if (Context->SecDir != NULL) {
    ...
    //
    Sha256HashAll (PkLe, 256, SignatureContext->PublicKeyHash);
  ...
  )
{
  ...
  //
  Sha256Ctx = AllocatePool(Sha256GetContextSize());
  if (Sha256Ctx == NULL) {
  ...
  **/
  ...
  Sha256Final (Sha256Ctx, Context->PeImageHash);
  FreePool (Sha256Ctx);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  for (Index = 0; Index < NUM_OF_PK; Index++) {
    if (CompareMem (PkDataBase[Index].Hash, SignatureContext->PublicKeyHash, 32) == 0) {
      ...
      if (RsaSetKey (RsaContext,
                     RsaKeyN,
                     PkDataBase[Index].PublicKey,
                     sizeof (PkDataBase[Index].PublicKey)) == 0) {
        RsaFree (RsaContext);
        return EFI_OUT_OF_RESOURCES;
      }
      if (RsaSetKey (RsaContext, RsaKeyE, mRsaE, sizeof (mRsaE)) == 0) {
        RsaFree (RsaContext);
        return EFI_OUT_OF_RESOURCES;
      }
      break;
  ...
  // Verify signature
  //
  if (RsaPkcs1Verify (RsaContext, Context->PeImageHash, SHA256_DIGEST_SIZE, SignatureContext->Signature, sizeof (SignatureContext->Signature)) == 1 ) {
